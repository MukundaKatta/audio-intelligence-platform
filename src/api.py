"""FastAPI application for audio-intelligence-platform."""
...
app = FastAPI(title="audio-intelligence-platform", version="0.1.0", description="audio-intelligence-platform API")
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "audio-intelligence-platform", "version": "0.1.0"}
...
async def process(req: ProcessRequest):
    ...
    try:
        ...
        resp = ProcessResponse(success=True, result=result, processing_time_ms=round(elapsed, 2),
                              metadata={"service": "audio-intelligence-platform", "input_length": len(req.input)})
...
@app.get("/status")
def status():
    return {"service": "audio-intelligence-platform", "version": "0.1.0", "ready": True,
